  ...
  return (
     <div className={styled.dropDownContainer}>
      <div
        tabIndex='0'
        onKeyDown={onEnter}
        className={styled.dropDownHeader} onClick={toggling}>
        {selectedOption || 
          <span className={styled.chooseOption}>Choose option
                            <IoIosArrowDown /></span>}
        </div>
        {isOpen && (
          ...
          <ul className={styled.dropDownList}
            tabIndex={-1}>
              {options.map(option => (
                ...
                className={styled.dropDownListItem} onClick={onOptionClicked(option)} key={option.id}
                  tabIndex={0}
                 >
                  {option.name}
